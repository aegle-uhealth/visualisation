...
shinyUI(dashboardPage(
  ...
  dashboardSidebar(
    ...
    #tags$style( word-break "break-all",
  ...
  dashboardBody(
    ...
    mainPanel(
      #verbatimTextOutput("id"),
            ...
            tabsetPanel(type = "tabs",
                  tabPanel("Graphs",
    # tags$head(tags$style(HTML('.skin-blue .main-sidebar .sidebar element.style{
    #                           word-break: break-all;
    # 
    #                           }'))),
    ...
    fluidRow(
      ...
      box(width = 4, uiOutput("Linesliders")),

      absolutePanel( top = 400, width = 120,  draggable = TRUE,
                     wellPanel(HTML("<strong> Visualizations </strong>"), 
                               ...
                               tags$style(type='text/css', "#linee { vertical-align: middle; height: 50px; width: 70px; font-size: 15px; background-color: #eee;}")
      ...
      #   
      # }),
      #conditionalPanel(condition = "input.v.doPlot == 'TRUE'" , box(width = 8, uiOutput("plots"))),
      #conditionalPanel(condition = "input.v.doLinePlot == 'TRUE'" , box(width = 8, uiOutput("Lineplots"))),
      box(width = 8, uiOutput("Lineplots")),
      tags$head(tags$script(src="loop11.js"))
      #box(width = 8, uiOutput("plots")),
      #box(width = 8, uiOutput("Lineplots"))
    )
      ),
    tabPanel("Table",
             # checkboxGroupInput("checkGroupTab", label = h3("Variables"),
             #                    choices = fooChoices,
             #                    selected = 1, inline = TRUE),
             dataTableOutput("DiaTable")),
             tags$head(tags$script(src="loop11.js"))
    )
  )
