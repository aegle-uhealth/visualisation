...
shinyServer(function(input, output, session) {
  ...
  while(length(content) != 0) {
    result[[i]]<-read.csv(textConnection(content), sep = ",", stringsAsFactors = FALSE, header=TRUE, fill=TRUE)[,c(1,64:82)]
  ...
  ts_status <- reactiveValues(a=0)
  if (local_df[, 1] == 'timestamp'){
    output$id <- reactive({ts_status$a + 1})
  }
  else{
    output$id <- reactive({ts_status$a})
  }
